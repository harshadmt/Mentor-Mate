...
const MySessions = () => {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        setIsLoading(true);
        const { data } = await axios.get('http://localhost:5000/api/videosession', { 
  ...
  const handleCancel = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/videosession/${id}`, { 
        withCredentials: true 
      });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `http://localhost:5000/api/videosession/${editingSession._id}`,
        {
      ...
      >
        ...
        {isLoading ? (
          ...
          >
            ...
            <h3 className="text-lg md:text-xl font-medium text-gray-700 mb-2">No sessions scheduled yet</h3>
            <p className="text-gray-500">Schedule your first session to get started</p>
          </motion.div>
        ) : (
